<?php
$obatList = $obats ?? [];
$totalJenis = $totalJenis ?? count($obatList);
$totalStok = $totalStok ?? array_sum(array_map(static fn($item) => (int) ($item['stok'] ?? 0), $obatList));
$hampirHabis = count(array_filter($obatList, static fn($item) => (int) ($item['stok'] ?? 0) <= 10));
$expiredCount = $expiredCount ?? count(array_filter($obatList, static function ($item) {
    $tanggalExpired = $item['tanggal_expired'] ?? null;
    if (empty($tanggalExpired)) {
        return false;
    }

    return strtotime($tanggalExpired) < strtotime(date('Y-m-d'));
}));
$expiringSoonCount = $expiringSoonCount ?? count(array_filter($obatList, static function ($item) {
    $tanggalExpired = $item['tanggal_expired'] ?? null;
    if (empty($tanggalExpired)) {
        return false;
    }

    $expiryDate = new DateTime($tanggalExpired);
    $today = new DateTime();
    $diff = $today->diff($expiryDate);

    return $expiryDate >= $today && $diff->days <= 90;
}));
?>
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-cyan-600 via-blue-700 to-indigo-800 p-7 text-white shadow-xl">
        ...
        <div class="relative flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <h1 class="text-3xl font-semibold leading-tight md:text-4xl">Data Obat</h1>
                <p class="mt-2 max-w-2xl text-sm text-blue-100 md:text-base">
                    Daftar inventori obat untuk memantau stok, kedaluwarsa, dan kebutuhan restock secara lebih cepat.
                </p>
            </div>
    ...
    <div class="flex flex-wrap gap-4">
        <article class="flex flex-1 items-center gap-4 min-w-[180px] rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-slate-800"><?= number_format((int) $totalJenis, 0, ',', '.') ?></p>
            </div>
        </article>
        ...
        <article class="flex flex-1 items-center gap-4 min-w-[180px] rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-slate-800"><?= number_format((int) $totalStok, 0, ',', '.') ?></p>
            </div>
        </article>
        ...
        <article class="flex flex-1 items-center gap-4 min-w-[180px] rounded-2xl border border-rose-100 bg-rose-50/40 p-4 shadow-sm transition hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-rose-700"><?= number_format((int) $hampirHabis, 0, ',', '.') ?></p>
            </div>
        </article>

        <article class="flex flex-1 items-center gap-4 min-w-[180px] rounded-2xl border border-red-100 bg-red-100/30 p-4 shadow-sm transition hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-red-800"><?= number_format((int) $expiredCount, 0, ',', '.') ?></p>
            </div>
        </article>

        <article class="flex flex-1 items-center gap-4 min-w-[180px] rounded-2xl border border-amber-100 bg-amber-50/50 p-4 shadow-sm transition hover:shadow-md">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-600">
                <i class="fas fa-clock"></i>
            </div>
            <div>
                <p class="text-[10px] font-bold uppercase tracking-wider text-amber-600">Segera Kedaluwarsa</p>
                <p class="text-xl font-bold text-amber-700"><?= number_format((int) $expiringSoonCount, 0, ',', '.') ?></p>
            </div>
        </article>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div class="relative w-full md:w-80">
                ...
                <input id="obat-search" type="text" placeholder="Cari nama obat, kategori, atau kode..." class="w-full rounded-xl border border-slate-300 bg-slate-50 py-2.5 pl-10 pr-3 text-sm text-slate-700 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100">
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody id="obat-table-body" class="divide-y divide-slate-200 bg-white text-slate-700">
                        ...
                        <?php foreach ($obatList as $index => $item): ?>
                            <?php
                            $expiryRaw = $item['tanggal_expired'] ?? null;
                            $expiryDate = !empty($expiryRaw) ? new DateTime($expiryRaw) : null;
                            $today = new DateTime();
                            $diff = $expiryDate ? $today->diff($expiryDate) : null;
                            $isExpired = $expiryDate ? $expiryDate < $today : false;
                            $isSoon = $expiryDate ? (!$isExpired && $diff->days <= 90) : false;
                            ?>
                            <tr class="transition hover:bg-blue-50/40">
                                ...
                                <td class="px-4 py-3 font-medium text-slate-800"><?= htmlspecialchars($item['kategori'] ?? '') ?></td>
                                <td class="px-4 py-3 font-medium text-slate-800">
                                    <div class="flex flex-col leading-tight">
                                        <span><?= number_format((int) ($item['stok'] ?? 0), 0, ',', '.') ?></span>
                                        <?php if (($item['stok'] ?? 0) <= 10): ?>
                                            <span class="text-[10px] font-bold uppercase text-rose-600">Re-stock!</span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-4 py-3 font-medium text-slate-800">Rp <?= number_format((int) ($item['harga'] ?? 0), 0, ',', '.') ?></td>
                                <td class="px-4 py-3">
                                    <div class="flex flex-col leading-tight">
                                        <span class="<?= $isExpired ? 'font-bold text-red-600' : ($isSoon ? 'font-semibold text-amber-600' : 'text-slate-700') ?>">
                                            <?= !empty($expiryRaw) ? date('d/m/Y', strtotime($expiryRaw)) : '-' ?>
                                        </span>
                                        <?php if ($isExpired): ?>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="/admin/obat/detail/<?= $item['id'] ?>" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-teal-200 bg-teal-50 text-teal-600 transition hover:bg-teal-100" title="Detail">
                                            <i class="fas fa-eye text-xs"></i>
                                        </a>
                                        <a href="/admin/obat/edit/<?= $item['id'] ?>" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-blue-200 bg-blue-50 text-blue-600 transition hover:bg-blue-100" title="Edit">
                                            <i class="fas fa-pen text-xs"></i>
                                        </a>
                                        <a href="/admin/obat/delete/<?= $item['id'] ?>" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100 handle-swal" title="Hapus">
