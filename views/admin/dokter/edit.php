...
<section class="mx-auto max-w-7xl space-y-6">
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-teal-600 via-cyan-700 to-blue-800 p-7 text-white shadow-xl">
        ...
        <div class="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-3xl font-bold leading-tight md:text-4xl">Edit Data Dokter</h1>
                <p class="mt-2 max-w-2xl text-sm text-blue-100 md:text-base">Perbarui data dokter agar informasi praktik dan kontak tetap akurat.</p>
            </div>
    ...
    <div class="grid gap-6 xl:grid-cols-[3fr_1fr]">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <form method="POST" action="/admin/dokter/update/<?= $formDokter['id'] ?? '' ?>" enctype="multipart/form-data" class="space-y-8">
                <input type="hidden" name="existing_images" value="<?= htmlspecialchars($formDokter['images'] ?? '') ?>">
                <div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-2">
                        <div>
                            ...
                            <input id="nip" name="nip" type="text" required value="<?= htmlspecialchars($formDokter['nip'] ?? '') ?>" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100">
                        </div>
                        <div>
                            <label for="sip" class="mb-2 block text-sm font-semibold text-slate-700">Nomor SIP</label>
                            <input id="sip" name="sip" type="text" required value="<?= htmlspecialchars($formDokter['sip'] ?? '') ?>" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100">
                        </div>
                        <div>
                            <label for="nama_dokter" class="mb-2 block text-sm font-semibold text-slate-700">Nama Dokter</label>
                            <input id="nama_dokter" name="nama_dokter" type="text" required value="<?= htmlspecialchars($formDokter['nama_dokter'] ?? '') ?>" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100">
                        </div>
                        <div>
                            <label for="spesialisasi" class="mb-2 block text-sm font-semibold text-slate-700">Spesialisasi</label>
                            <input id="spesialisasi" name="spesialisasi" type="text" required value="<?= htmlspecialchars($formDokter['spesialisasi'] ?? '') ?>" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100">
                        </div>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-200 pt-7">
                    ...
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        <div>
                            ...
                            <input id="no_telp" name="no_telp" type="text" required value="<?= htmlspecialchars($formDokter['no_telp'] ?? '') ?>" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100">
                        </div>
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">Kosongkan jika tidak ingin mengganti foto. Gambar otomatis di-crop 1:1 dan dikompres.</p>
                            <div class="mt-3">
                                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Preview Foto</p>
                                <img
                                    id="images-preview"
                                    src="<?= !empty($formDokter['images']) ? htmlspecialchars($formDokter['images']) : '' ?>"
                                    alt="Preview foto dokter"
                                    class="mt-2 h-24 w-24 rounded-xl border border-slate-200 object-cover cursor-zoom-in hover:scale-105 transition-transform <?= !empty($formDokter['images']) ? '' : 'hidden' ?>"
                                    data-image-preview="<?= !empty($formDokter['images']) ? htmlspecialchars($formDokter['images']) : '' ?>"
                        ...
                        <div>
                            ...
                            <select id="is_active" name="is_active" required class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-700 outline-none transition focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-100">
                                <option value="1" <?= (int) ($formDokter['is_active'] ?? 0) === 1 ? 'selected' : '' ?>>Aktif</option>
                                <option value="0" <?= (int) ($formDokter['is_active'] ?? 0) === 0 ? 'selected' : '' ?>>Non Aktif</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
                    ...
                    <ul class="mt-2 space-y-1">
                        <li class="flex items-start gap-2"><i class="fas fa-check-circle mt-1 text-emerald-500"></i><span>Periksa ulang NIP dan SIP.</span></li>
                        <li class="flex items-start gap-2"><i class="fas fa-check-circle mt-1 text-emerald-500"></i><span>Pastikan kontak bisa dihubungi.</span></li>
                        <li class="flex items-start gap-2"><i class="fas fa-check-circle mt-1 text-emerald-500"></i><span>Atur status praktik dengan benar.</span></li>
                    </ul>
                </div>
            </div>
        </aside>
...
<script>
    (function() {
        ...
        if (!modal || previewTriggers.length === 0) return;
        ...
        previewTriggers.forEach(trigger => {
            trigger.addEventListener('click', function() {
                const imageSrc = trigger.getAttribute('data-image-preview');
